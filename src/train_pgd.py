...
class Model:
    def __init__(self, model, exp_path, batch_size=128, num_workers=4, epochs=120, lr=0.1, device=torch.device("cuda"), **kwargs):
        ...
        self.attacker = torchattacks.PGD(self.model, eps=8/255, alpha=2/255, steps=7)
    ...
    def train(self, dataset_train, dataset_test):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            attack_acc = self.attack(dataloader_test)
            if sch:
                sch.step()
            e = time.time()
            time_epoch = e - s
            print(
                "Epoch: {} train_loss: {:.3f} train_acc: {:.2f}%, test_acc: {:.2f}%, attack_acc: {:.2f}% time: {:.1f}".format(
                    epoch, train_loss, train_acc * 100, test_acc * 100, attack_acc * 100, time_epoch
                )
            )
    ...
    def train_epoch(self, data_loader, opt, criterion):
        ...
        for (x, y) in data_loader:
            ...
            x = self.attacker(x, y).to(self.device)
            opt.zero_grad()
            pred = self.model(x)
...
def main():
    ...
    parser.add_argument('--exp_path', type=str, default='../weights/pgd')
