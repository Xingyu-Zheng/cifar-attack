...
def my_clip(raw, new, eps):
    delta = torch.clamp(new - raw, min=-eps, max=eps)
    return torch.clamp(raw + delta, min=0, max=1).detach()


def merge_1(attackers, weights, raw_images, labels, gauss_images):
    adv_images = torch.zeros_like(raw_images).detach()
    for attacker, weight in zip(attackers, weights):
        adv_images += attacker(raw_images, labels, gauss_images) * weight
    return adv_images


def merge_2(attackers, raw_images, labels, gauss_images):
    images = gauss_images
    for i in range(40):
        images = 0.3 * images + 0.7 * attackers[i%len(attackers)](gauss_images, labels, gauss_images)
        # images = 0.3 * images + 0.7 * attackers[i%len(attackers)](raw_images, labels, gauss_images)
    return images
...
def gaussian_kernel(device, kernel_size=15, sigma=2, channels=3):
    ...
    gaussian_filter = nn.Conv2d(in_channels=channels, out_channels=channels, padding_mode='replicate',
                                kernel_size=kernel_size, groups=channels, padding=(kernel_size-1)//2, bias=False)
